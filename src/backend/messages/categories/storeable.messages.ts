  ...
  private command = async (channel: string, user: string, message: string, msg: TwitchPrivateMessage) => {
    if (/^!((command)|(commands)|(cmd)|(cmds)) ((add .+)|(edit .+)|(delete .+)|(show .+)|(source .+)|(args .+)|(list))$/i.test(message)) {
      ...
      if (buffer.startsWith('add') || buffer.startsWith('edit')) {
        ...
        if (rand) for (let i of rand) buffer = buffer.replace(i, i.replace('rand', 'rnd'))
        ...
        if (exists) {
          ...
          ;(exists.cooldownPerUser = cooldownPerUser), (exists.mods = modsOnly)
          exists.streamer = streamerOnly
          await exists.save()
          let found = await from(this.commands)
            .pipe(
            ...
            .toPromise()
          if (found) {
            if (found.listener) found.listener.unbind()
        ...
        let found: any = await Command.findOne({ userId: this.iClient.userId, command: buffer })
        if (found)
          this.client.say(channel, `/me Here's the source for command "${buffer}": ${found.source.replace(/^!((command)|(commands)|(cmd)|(cmd)) ((add)|(edit))/i, '!cmd add')}`)
        else this.client.say(channel, `/me I couldn't find such command...`)
  ...
  private _generateListener = (command) => {
    return async (channel: string, user: string, message: string, msg: TwitchPrivateMessage) => {
      ...
      if (new RegExp('^' + tester, 'i').test(message)) {
        ...
        if (command.streamer && !msg.userInfo.isBroadcaster) return this.client.say(channel, `/me This command is for the streamer only...`)
        if (command.mods && !msg.userInfo.isBroadcaster && !msg.userInfo.isMod) return this.client.say(channel, `/me This command is for mods only...`)
        ...
        try {
          for (let i = 0; i < command.args.length; i++) {
            ...
            if (new RegExp(RGX_TARGET).test(arg)) {
              if (/\@streamer\s*/gi.test(arg)) {
                try {
                  ...
                  if (stream?.gameName) {
                    ...
                    status = 'Online'
                  } else if (channel.gameName) lastGame = channel.gameName
                  else lastGame = "'Never streamed before'"
              ...
              } else answer = answer.replace(`{{${i}}}`, shift())
  ...
  private flush = async (channel: string, user: string, message: string, msg: TwitchPrivateMessage) => {
    if (/^!((command)|(commands)|(cmd)|(cmds)) flush$/i.test(message)) {
      if (!msg.userInfo.isBroadcaster) this.client.say(channel, `/me Only the streamer can flush all commands from db... `)
        ...
        for (let c of this.commands) if (c.listener) c.listener.unbind()
        this.commands = []
