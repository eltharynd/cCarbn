...
export class Common extends Message {
  ...
  private cats = async (channel, tags, message, self) => {
    if (/!cat/i.test(message)) {
      ...
      if (facts && facts.length > 0) this.client.say(channel, `/me ${facts[Math.floor(Math.random() * facts.length)].text}`)
      else this.client.say(channel, `/me I'm trying to get some cool cat facts but this dudes aren't answering... I suppose that's what you get with free APIs`)
    }
  }
