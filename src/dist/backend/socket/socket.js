"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Socket = void 0;
var rxjs_1 = require("rxjs");
var socketIO = require("socket.io");
var express_1 = require("../api/express");
var mongo_1 = require("../db/mongo");
var chat_1 = require("../twitch/chat");
var Socket = /** @class */ (function () {
    function Socket() {
        var _this = this;
        Socket.io = new socketIO.Server(express_1.Api.server, {
            path: '/api/socket.io',
            transports: ['websocket']
        });
        Socket.io.on('connect', function (socket) {
            Socket.connections.push(socket);
            socket.emit('clientId', {
                clientId: mongo_1.Mongo.clientId,
            });
            socket.on('bind', function (data) {
                if (data.userId)
                    socket.join(data.userId);
            });
            socket.on('unbind', function (data) {
                if (data.userId)
                    socket.leave(data.userId);
            });
            socket.on('requestOBSlist', function (data) {
                if (data.userId)
                    socket.to(data.userId).emit('requestOBSlist', data);
            });
            socket.on('sendOBSlist', function (data) {
                if (data.userId) {
                    socket.to(data.userId).emit('receiveOBSlist', data.response);
                }
            });
            socket.on('elementsUpdated', function (data) {
                if (data.userId) {
                    socket.to(data.userId).emit('elementsUpdated', data.elements);
                }
            });
            socket.on('test', function (data) {
                if (data.userId) {
                    socket.to(data.userId).emit('test', data);
                }
            });
            //TODO build a queue not to repeat messages when user has multiple browsersources opened
            socket.on('chat-relay', function (data) { return __awaiter(_this, void 0, void 0, function () {
                var found;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            if (!(data.userId && data.message)) return [3 /*break*/, 2];
                            return [4 /*yield*/, (0, rxjs_1.from)(chat_1.Chat.clients).pipe((0, rxjs_1.filter)(function (c) { return c.userId === data.userId; }), (0, rxjs_1.take)(1)).toPromise()];
                        case 1:
                            found = _a.sent();
                            if (found) {
                                found.client.say(found.channel, data.message);
                            }
                            _a.label = 2;
                        case 2: return [2 /*return*/];
                    }
                });
            }); });
            socket.on('disconnect', function () {
                Socket.connections.splice(Socket.connections.indexOf(socket), 1);
            });
        });
    }
    Socket.addListener = function () {
    };
    Socket.removeListener = function () {
    };
    Socket.connections = [];
    return Socket;
}());
exports.Socket = Socket;
//# sourceMappingURL=socket.js.map