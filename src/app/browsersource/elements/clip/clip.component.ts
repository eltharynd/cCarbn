...
@Component({
  ...
  animations: [
    ...ELEMENT_ANIMATIONS_IN_INNER,
    ...ELEMENT_ANIMATIONS_OUT_INNER,
    trigger('delayed', [transition('void => FADE', [style({ opacity: 0 }), animate('5000ms ease', style({}))])]),
  ],
})
export class ClipComponent implements OnInit {
  ...
  constructor(private alerts: AlertsService, private sanitizer: DomSanitizer, public settings: SettingsService) {}
  ...
  ngOnInit() {
    ...
    if (!this.clip) {
      ...
      setTimeout(() => {
        this.element.src = this.sanitizer.bypassSecurityTrustResourceUrl(
          this.clip.embed_url +
