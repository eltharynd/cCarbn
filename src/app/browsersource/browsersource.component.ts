...
@Component({
  ...
  styles: ['.unloaded {overflow: hidden}'],
})
export class BrowserSourceComponent implements OnInit {
    ...
    this.router.events
      ...
      .subscribe(async (route) => {
        if (route) {
          ...
          if (this.auth.currentUser?._id) this.router.navigate([`${this.auth.currentUser?._id}/${route}`], { relativeTo: this.route.parent, replaceUrl: true })
  ...
  async ngOnInit() {
    if (this.userId) this.data.userId.next(this.userId)
    this.loaded = true
  }
}
