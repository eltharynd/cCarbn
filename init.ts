...
let getOauth = async () => {
  return new Promise(async (resolve) => {
    ...
    let oauthString = `https://id.twitch.tv/oauth2/authorize
            ?client_id=${twitch.clientId}
...
let init = async () => {
  ...
  if (fs.existsSync('twitch_credentials.json') || fs.existsSync('mongo_credentials.json')) {
    ...
    if (/y/gi.test(answer)) await prepareCredentials()
    else console.log('\x1b[33m%s\x1b[0m', `Using existing credentials...`)
  } else {
    fs.copyFileSync('src/assets/templates/twitch_template.json', 'twitch_credentials.json')
    fs.copyFileSync('src/assets/templates/mongo_template.json', 'mongo_credentials.json')
  ...
  if (/y/gi.test(answer)) {
    if (fs.existsSync('endpoint_credentials.json')) {
      ...
      if (/y/gi.test(answer)) fs.copyFileSync('src/assets/templates/endpoint_template.json', 'endpoint_credentials.json')
    } else fs.copyFileSync('src/assets/templates/endpoint_template.json', 'endpoint_credentials.json')
  ...
  if (/y/gi.test(answer)) {
    ...
    if (/y/gi.test(answer)) {
      ...
      if (data) {
        ...
        } else console.log('\x1b[31m%s\x1b[0m', 'Could not connect to mongodb... Please check your credentials...')
      } else console.log('\x1b[31m%s\x1b[0m', 'Could not retrieve a token... Please check your credentials...')
    } else {
      console.log(
        '\x1b[33m%s\x1b[0m',
        `Unfortunately there is no way to get a chat user token without confirming from the browser... You will have to do this on your local machine, after that the token will be saved to the database...`
      )
  ...
  } else console.log('\x1b[33m%s\x1b[0m', `Using existing oauth...`)
